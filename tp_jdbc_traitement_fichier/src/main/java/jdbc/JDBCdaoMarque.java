...
public class JDBCdaoMarque implements IMarqueDao {
	...
	/**
	 * Insert la ingrédient placée en paramètre si elle n'est pas déjà enregistrée dans la BDD
	 *@param Objet Marque
	 *@return l'ID de la ingrédient inserrée si effecutée correctement,sinon retourne un int >= 1 correspondant à l'id_Cat dans la BDD 
	 **/
	...
	public Marque selectMarque(String nomMarque) {
		...
		try {
			...
			PreparedStatement insertMarque = connection
					.prepareStatement("SELECT * FROM `marque` WHERE nom_marque= ?");
	...
	public Marque selectMarque(int idMarque) {
		...
		try {
			...
			PreparedStatement insertMarque = connection
					.prepareStatement("SELECT * FROM `marque` WHERE id_marque= ?");
	 ...
	 * 
	 * @param nomMarque
	 * @return Retourne l'id_Cat en BDD de la Marque dont le nom est égal au
	 *         param
	 */
